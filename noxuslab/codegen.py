...
The output is a top-level script (no `def main`, no `__future__`) that
re-creates the workflow with the SDK and prints the new id. It is
deliberately small and readable; non-trivial node configs over a length
threshold are hoisted into module-level variables.
"""
...
def _topo_order(node_ids: list[str], edges: list[dict]) -> list[str]:
    """Kahn topological sort. Returns ids in topo order; falls back on cycle."""
...
def _to_wire(wf: Any) -> dict:
    """Coerce a WorkflowDefinition or a raw wire dict into the wire dict shape."""
...
def workflow_to_python(
    ...
    *,
    ...
    `wf` may be a `WorkflowDefinition` instance or a wire dict
